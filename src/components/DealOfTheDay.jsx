...
export async function loader() {
    try {
        ...
        console.error('SERVER: Error fetching deal, using fallback data.', error);
        // If fetch fails during SSR/Build, we safely return fallback data instead of crashing.
        return json({ deal: fallbackDeal });
    }
...
export default function DealOfTheDay({ initialDeal }) {
    // The component now uses the data passed from its parent (HomePage)
    const deal = initialDeal;

    const [timeLeft, setTimeLeft] = useState({ hours: 0, minutes: 0, seconds: 0 });
    
    // Set up timer (This runs only on the client, which is safe)
    useEffect(() => {
    ...
    }, [deal]); // Re-run when deal data changes

    const updateTimeLeft = () => {
        // This function must be defined inside useEffect or use useCallback, 
        // but for simplicity we keep it outside and check for window existence.
        if (typeof window === 'undefined' || !deal || !deal.expiresAt) return;
